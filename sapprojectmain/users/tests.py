...
class TestUserDetailSecurity(BaseTestCase):
    def test_user_can_access_other_user_data(self):
        """
        🚨 This test exposes a potential IDOR vulnerability
        """
        self.auth(self.user)
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        # This SHOULD probably be restricted depending on your policy
    ...
    def test_staff_cannot_modify_superuser(self):
        ...
        res = self.client.put(
            reverse("user-detail", args=[self.superuser.id]),
            {"username": "hacked"}
        )
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)
...
class TestToggleUser(BaseTestCase):
    def test_staff_can_toggle_user(self):
        ...
        res = self.client.patch(
            reverse("user-toggle", args=[self.user.id])
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_staff_cannot_toggle_superuser(self):
        ...
        res = self.client.patch(
            reverse("user-toggle", args=[self.superuser.id])
        )
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)
...
class TestCurrentUser(BaseTestCase):
    ...
    def test_change_password_wrong_old(self):
        ...
        res = self.client.put(reverse("user-me"), {
            "old_password": "wrong",
    ...
    def test_user_delete_self(self):
        ...
        res = self.client.delete(reverse("user-me"))
